...
(function () {
  ...
  /* Un évènement de clavier ou de focus peut viser le document lui-même, qui
     n'a pas de `matches` : sans cette garde, la page cassait sur une touche
     pressée hors de tout champ. */
  const estChamp = n => !!(n && n.matches && n.matches("[data-qty]"));

  /* Sélection du contenu au clic, pour taper directement par-dessus. */
  document.addEventListener("focusin", e => {
    if (estChamp(e.target)) e.target.select();
  });

  document.addEventListener("input", e => {
    const f = e.target;
    if (!estChamp(f)) return;
  ...
  function patchCard(id) {
    ...
    const wasTyping = document.activeElement && card.contains(document.activeElement) &&
      estChamp(document.activeElement);
