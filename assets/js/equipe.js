...
(function () {
  ...
  function renderCard() {
      ...
      '<div class="panel">' +
        '<div class="staffhead" style="--role:' + esc(r.color) + '">' +
          ...
          /* Une fiche archivée garde ce qui s'est passé — carrière, heures,
             avertissements — mais ce qu'on a écrit dessus reste corrigible :
             un nom mal orthographié l'est toujours, et un motif de départ
             choisi trop vite aussi. */
          (MNStore.estArchive(u)
            ? (canEdit
                ? '<div class="staffhead__acts">' +
                    ...
                    '<button class="btn btn--ghost" id="c-depart">' + svg("logout") +
                      "<span>Corriger le départ</span></button>" +
                  "</div>"
                : "")
            : (canEdit || canWarn
              ? '<div class="staffhead__acts">' +
    ...
    const dp = $("#c-depart"); if (dp) dp.addEventListener("click", () => archiver(u, true));
  ...
  function bandeauDepart(u) {
    ...
    return '<div class="depart">' +
      ...
      '<p class="hint">Cette fiche est conservée : ancienneté, carrière, formations ' +
        "et avertissements restent lisibles. Ce qui s'est passé ne bouge plus, mais " +
        "ce qui est écrit dessus se corrige encore.</p>" +
    "</div>";
  }
  ...
  function archiver(u, corriger) {
    ...
    body.innerHTML =
      '<p class="hint">' + (d0
        ...
        : esc(u.pseudo) + " quittera l'équipe et ne pourra plus se " +
      ...
      '<div class="field"><label class="label">Motif</label>' +
        '<div class="motifs" id="d-motifs">' + MNStore.MOTIFS_DEPART.map(m =>
          '<button type="button" class="motif' + (m.id === motifDepart ? " is-on" : "") +
      ...
      '<div class="field" style="max-width:220px"><label class="label" for="d-date">Date du départ</label>' +
        '<input class="input" id="d-date" type="date" value="' + esc((d0 && d0.le) || auj) +
          '" max="' + auj + '"></div>' +
      '<div class="field"><label class="label" for="d-note">Précisions (facultatif)</label>' +
        '<textarea class="textarea" id="d-note" maxlength="600" ' +
          'placeholder="Ce qu\'il faut retenir de ce départ…">' +
          esc((d0 && d0.note) || "") + "</textarea></div>";
    ...
    MNUI.modal({
      title: (d0 ? "Corriger le départ de " : "Archiver ") + u.pseudo, body,
      actions: [
        { label: "Annuler", variant: "btn--ghost", onClick: c => c() },
        {
          label: d0 ? "Enregistrer" : "Archiver la fiche",
          variant: "btn--primary", icon: d0 ? "check" : "logout",
          onClick: async close => {
            const depart = {
            ...
            close();
            MNUI.toast(d0
              ? "Départ corrigé" + suite(r)
              : u.pseudo + " est archivé — sa fiche reste consultable" + suite(r), "ok");
          }
        }
      ]
    });
  ...
  function ligneAvert(a, u, peut) {
    ...
    return '<div class="av' + (actif ? "" : " is-off") + '" style="--grav:' + esc(g.couleur) + '">' +
      ...
      (peut
        ? '<div class="av__acts">' +
            '<button class="btn btn--icon" data-av="modifier|' + esc(a.id) +
              '" title="Corriger cet avertissement">' + svg("edit") + "</button>" +
  ...
  function avertir(u, dejaLa) {
    const auj = MNDuty.jourLocal();
    const jourDe = a => String(a && a.at || "").slice(0, 10) || auj;
    ...
    body.innerHTML =
      ...
      '<div class="field"><label class="label">Gravité</label>' +
        '<div class="gravites" id="a-grav">' + MNStore.GRAVITES.map(g =>
          '<button type="button" class="grav' + (g.id === depart ? " is-on" : "") +
      ...
      '<div class="field"><label class="label" for="a-motif">Motif</label>' +
        '<input class="input" id="a-motif" maxlength="120" value="' +
          esc((dejaLa && dejaLa.motif) || "") + '" ' +
      ...
      '<div class="field"><label class="label" for="a-note">Précisions (facultatif)</label>' +
        '<textarea class="textarea" id="a-note" maxlength="600" ' +
          'placeholder="Ce qui s\'est passé, ce qui est attendu ensuite…">' +
          esc((dejaLa && dejaLa.note) || "") + "</textarea></div>" +
      '<div class="field"><label class="label" for="a-date">Date des faits</label>' +
        '<input class="input" id="a-date" type="date" value="' + esc(jourDe(dejaLa)) +
          '" max="' + auj + '">' +
    ...
    MNUI.modal({
      title: (dejaLa ? "Corriger l'avertissement de " : "Avertir ") + u.pseudo, body,
      actions: [
        { label: "Annuler", variant: "btn--ghost", onClick: c => c() },
        {
          label: dejaLa ? "Enregistrer" : "Donner l'avertissement",
          variant: "btn--primary", icon: dejaLa ? "check" : "alert",
          onClick: async (close, b2, btn) => {
            ...
            if (dejaLa) {
              ...
              if (!rc.ok) {
                btn.disabled = false;
                return MNUI.toast("Enregistrement impossible : " + rc.error, "err");
              }
              close();
              return MNUI.toast("Avertissement corrigé" + suite(rc), "ok");
            }
